...
The unmodified version of FLAMS is also available as a web service at https://www.biw.kuleuven.be/m2s/cmpg/research/CSB/tools/flams/ .

## System requirements

Linux 64-bit, Windows, and Mac OS are supported.
...
The UniProt version is not integrated into conda or PyPI. To install, clone this repository, create a conda environment with all the dependencies (can use the conda environment of the original FLAMS or create a new conda environment with `conda create -n EnvironmentName python=3.10`, activate it and install all dependencies with `pip install -r ./PathToLocalFLAMS/requirements.txt`), and run `python -m pip install ./PathToLocalFLAMS` (or if you plan on making edits to the code later on, run `python -m pip install -e ./PathToLocalFLAMS`).
...
Make sure to have an internet connection when running FLAMS for the first time. It is needed for the database download from UniProt.
...
* one of:
  ...
  * `batchFilePath`, used with `--batch`, is the path to a tab-separated file for batch runs. The file should contain 1 entry per line, with UniProt IDs in the 1st column, and positions in the 2nd column.
...
Optional arguments:
* `errorRange` is a number of positions before and after `position` to also search for modifications. [default: 0]
...
With the following commands, you search for what modification the Q at the 6th position of the *Homo sapiens* Histone H3.1 (UniProt ID: P68431) has. 
...
°: the window can be smaller than the [-5;+5] window if the sequence alignment ends sooner, which can happen for modified sites near the start/end of the protein.
...
If you wish to use other filters, you can modify the *uniprot.py* script (valid_ECO_codes for the ECO code filter and/or the url in download_with_rest_api() for existence code). You can make changes in the MODIFICATIONS dictionary in *setup.py* to add/remove modification types, edit amino acid lists, or the regular expressions used to group entries. All changes should be paired with a change of the version number in *setup.py* to download a new version of the database.

### Supported PTM types

An overview of the PTM types, how to call them in FLAMS, and on which amino acid they can be found° is given in the table below. This table can also be found as a tab-separated file named FLAMS_supported_ptms_v20.txt .
...
°: for PTM types that exist in the CPLM/dbPTM version of FLAMS, the list of amino acids that could carry the modification type was left unchanged; however, for the newly added types, the list was inferred from the downloaded entries that were sorted into that type and might not be biologically relevant. You can update the list in MODIFICATIONS in *setup.py*.
